		 ...
		 ++iCount)
	{
		...
		while (1)
		{
			...
			if (aIterator==pNode->aMap.end())
				...
				else // If we have a failure node
				{
					//What is the depth difference?
					unsigned short usDepth;
					usDepth=pNode->usDepth - pNode->pFailureNode->usDepth-1;
					...
					//This is how many chars to remove
					sMatchedString=sMatchedString.substr(usDepth,
						sMatchedString.length()-usDepth);
		...
		if (pNode->bFinal)
		{
			...
			//Go back
			//iCount-=(sMatchedString.length()-1);  // Removed by elfinhe@gmail.com, add this if you want to match inner substring.
			pNode=&m_aRoot; // Added by elfinhe@gmail.com
			bSwitch=false; // Added by elfinhe@gmail.com
